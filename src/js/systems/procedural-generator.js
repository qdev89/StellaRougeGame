...
class ProceduralGenerator {
    ...
    generateSector(sectorNumber) {
        ...
        const sector = {
            ...
            hazards: [],
            specialEncounters: [],
            bossEncounter: this.generateBossEncounter(sectorNumber)
        };
        
        // Determine number of enemies based on sector difficulty
        const minEnemies = Math.floor(CONSTANTS.SECTOR.MIN_ENEMIES * this.difficultyMultiplier);
        const maxEnemies = Math.floor(CONSTANTS.SECTOR.MAX_ENEMIES * this.difficultyMultiplier);
    ...
    addHazardsToSector(sector) {
        // Determine number of hazard patterns based on sector length and frequency
        const hazardCount = Math.floor(
            (this.sectorLength / 1000) * CONSTANTS.SECTOR.HAZARD_FREQUENCY * this.difficultyMultiplier
        );
